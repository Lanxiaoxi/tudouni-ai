...
# 一个回合最多几步。默认 **120**：实测把这个项目最典型的长任务（"参考现有实现加一个
# 工具"）跑到收尾需要 19 步，而 20 只剩最后一步的余量 —— 任何一次返工都会撞墙。
# 80 在"返工两次、再中间多聊两句"时也会撞上，所以又抬了一档到 120；而步数用尽是
# **可续的**（会话完好，接着跑就是），宁松不紧。
#
# 它在这里而不是藏在 `Agent.run` 的签名里，是因为 `init` 要把它发给前端：
# 前端要显示"第 3/120 步"，而它显示一个和实际预算不同的数比不显示更坏。
...
class Runtime:
    ...
    **唯一的例外是模型，而且它是被记账的例外**（见 `select_model`）：换模型在会话历史里
    ...
    # `init` 要把它发给前端显示（"第 3/120 步"），而前端显示一个和实际预算不同的数
