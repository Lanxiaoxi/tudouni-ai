...
tudouni --tui --theme 墨绿仪器    # 换配色。13 套：a 石墨琥珀是默认，也能给名字/序号
tudouni --tui --theme a-t        # 它的透明版：对话区不涂底，露出终端自己的底板
tudouni --tui --theme a-t2       # 再深一档：连顶栏 / 状态栏 / 输入框和开场那三个框也不涂（框线仍是主题色）
...
键位和 13 套配色的来历见 `doc/TUI-design.md` 第十四节。
...
/quiet      安静模式开关（见「TUI 的安静模式」）；不带参数切换
```
...
```powershell
tudouni --tui --quiet     # 起手就安静；运行中还能用 /quiet 切
```

它换的是**这个界面怎么画**（不进协议、不影响 runtime），一次工具调用从三行
（调用 / 权限 / 结果）压成一行：
...
- **结果回填到同一行**（按 `call_id` 配对，不是按顺序）：所以并行批次里两条一模一样的
  `read_file a.py` 也不会互相贴错。失败/被拒绝是同一行上的 `✗`，不是另起一行。
- **主要内容按工具挑**：`read_file`/`write_file` 是路径、`shell` 是命令、`grep` 是模式、
  `fetch_web` 是 URL、`ask_user` 是问题本身；MCP 那些外部工具取第一个像样的参数。
  风险**靠颜色不靠文字**（和逐条显示同一个取向）。
- **静默那几种放行**（等级自动 / autopilot / 按过 `t` / 命中命令前缀规则）不再各占一行；
  而**等你批准、被拒绝、被策略禁止**照旧显示 —— 它们是"这一轮为什么停在这儿"唯一的出口。
- **思考过程只留一行**，带转圈和实时字符数：`▸ 思考过程 ⠇ 1,284 字符`。
  转圈是**防"卡死"误判**用的（安静模式下屏幕上没有别的东西在动，而一次模型往返是秒级）：
  它只在回合真的在跑时转，**等你按键时停下**，回合收尾时定格成
  `▸ 思考过程（1,284 字符 · Ctrl+T 展开）` —— `Ctrl+T` 照旧能展开全文。
- 状态栏右边多一格 `安静`（**只开着时占位**，所以关掉它时那一行和加这个功能之前一个
  字符都不差）。`/quiet on` / `/quiet off` 可以直接设成那个值。
- **只影响之后画出来的东西**：已经画在屏幕上的回合是当时按当时的模式画的（回声里明说
  这一句）。想整屏都是安静模式，就 `--quiet` 起来。
- **它和 `/autopilot` 不是一类**：`/autopilot` 改的是 runtime 的行为，所以界面要等它
  回话才敢显示；`/quiet` 是这个界面的显示偏好 —— 界面就是真值来源，所以当场生效、
  一条协议消息都不发。
...
frontends/          界面：**各前端之间不共享代码**，只讲协议
  ...
  tui/               Python + Textual
    ...
    theme.py           13 套配色（**纯数据，不 import textual**；派生角色按固定规则算，
                       其中 `A-T` / `P3-T` / `A-T2` 三套是透明版：底交给终端自己，
                       最后那套连横栏和开场那三个框也交出去）
...
属于审计、不属于输出的原因。默认步数 **120**（`runtime/composition.py` 的
`DEFAULT_MAX_STEPS`，`init` 会把它发给前端，所以它只在这一个地方定义）：实测把这个
项目最典型的长任务（"参考现有实现加一个工具"）跑到收尾需要 19 步，留到 120 是给
...
一行 `[配置] 忽略了 …：这个程序现在不读任何环境变量、也不读 .env；密钥写在 ~/.tudouni/config.json
里那条路由的 "api_key" 上` —— 不读可以，不出声不行
...
- **`doc/` 里的文件。** `guide.md` 是本项目最早那份分阶段设计文档；`summary.md` 是
  ...
  **第十四节是 v2 的界面改版**（九张界面稿 + 13 套配色的落地，以及三处和早先决策的
  冲突按什么办），
...
- **TUI 上明确没做的那几件**（免得被当成漏了，理由见 `doc/TUI-design.md` 第 14.5 节）：
  ...
  （`P3 粉紫` 能选、能跑，但没单独校过；它的透明版 `P3-T` 同理）
