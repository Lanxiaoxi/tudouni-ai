...
tudouni --tui --theme 墨绿仪器    # 换配色。12 套：a 石墨琥珀是默认，也能给名字/序号
...
键位和 12 套配色的来历见 `doc/TUI-design.md` 第十四节。
...
frontends/          界面：**各前端之间不共享代码**，只讲协议
  ...
  tui/               Python + Textual
    ...
    theme.py           12 套配色（**纯数据，不 import textual**；派生角色按固定规则算，
                       其中 `A-T` / `P3-T` 两套是透明版：底交给终端自己）
...
- **`doc/` 里的文件。** `guide.md` 是本项目最早那份分阶段设计文档；`summary.md` 是
  ...
  **第十四节是 v2 的界面改版**（九张界面稿 + 12 套配色的落地，以及三处和早先决策的
  冲突按什么办），
