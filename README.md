...
- **`fetch_web` 不能是 LOW。** LOW 是自动放行档（默认 `auto_approve=("low",)`），而它是
  ...
  调用就能把 `read_file` 读到的东西发出去，全程没人看见。原先那三个 LOW（`read_file` /
  `list_files` / `get_current_time`）之所以担得起 LOW，正是因为它们只读本地、且读不出
  工作区 —— 这条边界从 `fetch_web` 开始才第一次被打破。
...
- **联网工具不进并行批次。** 注册期硬校验要求 `parallel_safe` 的工具必须是 LOW，所以
  medium 的 `fetch_web` 天然进不去；`web_search` 虽然是 LOW，但它会阻塞在网络上，v1 也
  没给它声明 —— 真要吃下那笔收益，得先有"按域名放行的那一批只读抓取"，那时一并设计。
  代价说明白：一次"抓 5 个 URL"的任务是串行的，5 个 300ms 的网页就是 1.5 秒，相对整个
  回合（模型往返是秒级）可以接受。
