...
- `.tudouni/` 是这个运行时**自己的**位置（和权限策略 `.tudouni.json` 同一个家）；
...
- **一次研究任务要被问很多次。** `fetch_web` 是 medium，默认每次都问，而"查一次资料"
  ...
  `.tudouni.json` 里 `{"auto_approve_tools": ["fetch_web"]}`，或者在审批时按 `t`。
...
| `.tudouni/skills/`、`.agents/skills/`、`.skills/` | **项目级技能**：模型按需读取的步骤文件（`SKILL.md`） | **是** —— 它是给人写、给人 review 的内容，而且 agent 不许写它 |
| `~/.tudouni/skills/` 等 | **个人级技能**（在工作区外面，优先级高于项目级） | 你本机的事，随你 |
| `.sessions/` | 会话状态（含工具读到的文件正文） | 否 |
| `.logs/` | 审计轨迹（含工具参数预览） | 否 |

这几样都是本机的东西，不是源码 —— `.gitignore` 里全排除了。
`.env.example` 是例外：它是给人看的模板、不含真密钥，所以**它是要提交的**（`gitignore`
里那条 `.env.*` 后面跟了一句 `!.env.example` 把它重新包含回来）。**技能目录是第二个
例外**：它是给人写、给人 review 的内容，所以 `.tudouni/`（以及 `.agents/`、`.skills/`）
不进 `.gitignore` —— 和下面那条正好相反，而两者之所以不同，正是因为"谁能写它"不同：
`.tudouni.json` 是程序自己会改的文件，技能只有人能改。
...
- **`.git/` 仍然在 agent 的可写范围内。** `.git/config` 的 `core.pager` /
  ...
  东西，而且不需要经过 shell 审批。控制面拒绝表（`.tudouni.json` / `.tudouni/` /
  `.sessions/` / `.logs/`）目前不含 `.git/`。
