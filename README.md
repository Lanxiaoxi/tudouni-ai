...
出来**，所以它和 `--audit` 报的数字对得上。老 CLI（`uv run main.py`）里也有同样五条，
答案打到 stderr、格式是行式的。
...
* **工具 schema 每轮都要发出去。** 内置工具合计约 8600 字符（默认装配的 13 个 —— 其中后台
  ...
  都明显变贵。这个运行时选择"配了就连"，所以规模靠 `mcp.json` 里少配几个来控制。
