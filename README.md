...
权限范围、本次会话 —— 它默认收起，**任务列表一出现就自动顶开一次**，之后 `Ctrl+B`
说了算；窄屏收起时降级成一行摘要），
...
* **工具 schema 每轮都要发出去。** 内置工具合计约 6000 字符（默认装配的 9 个），而一个
  server 动辄十几个工具 —— 挂三四个 server 会让每一轮请求都明显变贵。这个运行时选择
  "配了就连"，所以规模靠 `mcp.json` 里少配几个来控制。
