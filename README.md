...
**想看"它现在什么状态、手里有什么工具、在用哪个模型"，不用离开界面**：

```
/status     模型、会话、工作区、工具个数、累计 token、轮次与调用次数、审计路径
/tools      每个工具 + 风险 + 会不会问你（/model 那张清单也在这里）
/model      不带参数列出可选模型；/model deepseek-v4-pro 直接换
```

`/status` 和 `/tools` 都是**只读**的，答案直接进会话流（不弹面板 —— 那一屏最有用的时候
恰恰是"它跑着、我想看一眼花了多少"，弹层会把你要看的东西盖住）。`/status` 里那几笔账
**从审计日志数出来**，所以它和 `--audit` 报的数字对得上。老 CLI（`uv run main.py`）里
也有同样三条命令，答案打到 stderr，格式是行式的。
...
- **`上下文 2.0k/1M（0.2%）` 是"上一次请求实际发出去多少"**，不是"现在"：下一次请求要加上
  ...
  `config.CONTEXT_WINDOWS`，而那张表由 `state/model.py` 的模型目录派生**（见「换模型」），
  因为响应里没有这个字段。表里没有的模型名
...
```powershell
uv run main.py                      # 进去之后：/model 看清单、/model deepseek-v4-pro 换
uv run main.py --tui                # 同上，TUI 里那一屏会列 label、说明和上下文窗口
$env:DEEPSEEK_MODEL = "deepseek-v4-pro"   # 改默认（下一次新会话生效）
```

**哪些模型是"认识的"只有一处定义**：`state/model.py` 的 `MODEL_CATALOG`（当前是官方
「模型 & 价格」那页上的两个名字：`deepseek-flash` 和 `deepseek-v4-pro`）。`/model` 那份
清单、`CONTEXT_WINDOWS` 那张窗口表、"这个名字认不认识"这个判断，三处读的都是它 ——
各写一份的话，漂掉的方式是静默的（清单里列着它，选了之后状态栏却报不出占比）。

**旧模型名认，但不列进清单。** `deepseek-v4-flash` / `deepseek-v4-flash-vision-exp`
按官方说明已经下线、请求由 V4.1-Flash 提供服务，所以它们是能调用的**别名**，不是能选的
东西：`DEEPSEEK_MODEL` 里写着它们照样能用（折算到 `deepseek-flash`），但 `/model` 不会
摆出两个效果一样、价钱也一样的选项。

### 三条不能商量的

1. **名字必须精确，打错不猜。** `/model flsh` 得到的是"目录里没有这个模型"外加清单。
   这不只是风格：配色选错一眼看得出来，而模型选错只会在账单上体现（Pro 的未命中输入
   是 Flash 的四倍多）。**目录外的名字一律拒绝**，包括看起来像另一个网关的模型名 ——
   这个项目只有一个 base_url、一把密钥，"换到另一个网关"没法表达，所以那种情况是拒绝
   而不是假装切成功了。
2. **选择跟着会话走，不是改全局默认。** 换完之后那个选择写进 `session.metadata`，
   所以恢复会话时还是它（启动时会用一行 `[模型] 这个会话选的是 …` 说明），而**新会话
   回到 `.env` 里那个**。想改默认就改 `DEEPSEEK_MODEL` —— 一次 `/model` 悄悄改掉全局
   默认，和 `--autopilot` 那条"一次性的决定不该变成永久默认"是同一个担心。
3. **换模型在一轮跑着的时候也允许，但本轮不受影响。** 那个回合的请求已经发出去了，
   ...
   —— 这条记录此后每一轮都会重发，所以"这段是三块钱一次的模型写的"变成会话里可查的
   事实，而不是只有审计日志里那一串 `model_call` 才知道的事。
...
而这一格是给查账的人读的。换模型这个动作本身不单独记一条事件：它落在两处可查的事实里
（`init` / `ui(state)` 的模型名，以及此后每条 `model_call` 的名字）。

### 未知模型名怎么办

指向自建网关（`DEEPSEEK_BASE_URL`）时，`DEEPSEEK_MODEL` 可以是任意名字。那种情况下
`/model` 那张清单仍然只列目录里的名字（它列的是"能选的"，不是"能用的"），而状态栏
**只报用量、不报占比** —— 错的百分比比没有百分比更坏。启动时会提示一句该往哪加。
