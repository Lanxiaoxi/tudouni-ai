...
#
# safe_path 管的是"别出去"，管不了"有些地方别进来"。这些东西决定的都不是任务
# 内容，而是**这次运行本身可不可信**。全部私有数据住在**一个目录**里（每个工作区一份）：
#
#   .tudouni/
#     permissions.json   权限策略 —— 能写它就等于能给自己发权限（把 shell 加进免审批名单）
#     sessions/          会话历史 —— 能写它就能伪造"用户批准过"的记录
#     logs/              审计日志 —— 能写它就能抹掉"谁批准了什么"的证据
#     skills/            技能 —— 能写它就能改自己此后每一轮的指令
#
# 为什么是"守一个目录"而不是"点名四个路径"：这四样是同一个东西的四个部分（这次运行
# 自己的状态），而**往里加东西的时候不需要再改这张表** —— 新加的文件天生就在里面。
# 逐个点名会漏，和 auto_approve 不收 "high" 是同一个取向：规则的作用范围必须一眼看得懂，
# 而且不能随着将来新增的东西自动变宽。
#
# **技能那一项是最不显眼、后果却最持久的。** 技能正文加载后会拼进此后每一次请求，
# 所以能写它就等于能改自己接下来每一轮的指令，而且改一次永久生效 —— 比改权限策略更
# 隐蔽：策略改了至少还有一条 permission 事件，而"我给自己加了三条规矩"在审计里什么都
# 看不出来。
#
# **旧位置仍然留在表里**（.tudouni.json / .sessions / .logs）：程序已经不再读写它们，
# 但它们可能还在磁盘上、装的是同一类东西 —— 漏掉就等于给"降级回旧布局"留了一条能写的路。
#
# 只挡写，不挡读：读不出这些文件本身没有破坏性（技能正文还必须读得到），而 agent 看不懂
# 自己为什么被拒时只会反复重试。
CONTROL_PLANE = (".tudouni.json", RUNTIME_DIR_NAME, ".sessions", ".logs")
...
class FileSystem:
    ...
    def writable_path(self, path: str) -> Path:
        ...
        `.resolve()` 已经把符号链接展开了，所以"先建一个指向 .tudouni/ 的软链再写"
        这条弯路也会落到同一张表上。
        """
