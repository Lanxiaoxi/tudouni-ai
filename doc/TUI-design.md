...
| 5 | **上下文栏（左栏）** | 任务 / 已加载技能 / 权限范围 / 本次会话四块，**外加后台任务一块**（后台是后加的，排在最后，见 14.3）。**数据来自新加的 `ui(kind:"state")`**，见 14.3 |
...
| `/model` | `state/model.py` 的目录 + `Runtime.select_model()` | 不带参数列清单（`render_models`）；带参数发 `set_model`，等回包 |
...
| `/effort` | `Runtime.select_effort` + `init.effort_levels` | 不带参数列档位；档位**由 runtime 给**，界面不写死 |
