...
| 命令 | 数据从哪来 | 界面形态 |
|---|---|---|
| `/theme` | `theme.py` 的 14 套（`ORDER` / `THEMES`） | **不带参数弹选择面板**：`↑↓` 选、`Enter` 换、`Esc` 取消；带参数照旧走 `theme.resolve` |
...
**`/theme` 原本在 18 的草稿里被排除**（理由是"14 套各有中文名、换错了一眼看得出来"）。
后来一起做了，因为那句话只否掉了一半：配色换错确实看得见，但**清单上那些名字本来也只是
为了照着打一遍** —— 14 套里的序号最容易记错（`/theme 10` 是 `A`，不是 `P10`）。而
"看得见"这条差别没消失，它落在 18.2 那张表的**关不关面板**那一格里。
...
- **工具描述与工具结果**（模型看的）；
- **14 套配色的中文名**：它们是长在主题上的数据，英文名并存（`name_en` / `source_en`），
