...
| 8 | **配色层** | 12 套主题（见 14.2），`/theme` 运行中换 |
...
**透明版（`A-T` / `P3-T`）**：很多 CLI 的 TUI 看起来是"透"的 —— 终端底板什么色，
对话区就什么色。做法是把那套配色的 `bg` 换成 `ansi_default`（"用终端自己的底色"）：

| 哪一格 | 透明版 | 为什么 |
|---|---|---|
| `bg`（窗口底 / 对话区） | **不涂**（`ansi_default`） | 这一大片透出来，就是"终端底板什么色、它就是什么色" |
| `chrome` / `surface` / `rail` / `elevated` / `sunk` | **照旧实色** | 顶栏、状态栏、输入框、左栏、命令面板、思考块靠底色分层；全透了就只剩文字和边框 |
| 派生角色（rail_bar / hairline / …） | 按**原版那套的 `bg`** 算 | `ansi_default` 不是一个能插值的色值；而且这样"透明版"和"原版"的差别只剩最底下那一大片 |
...
得同时成立四件事 —— `theme.py` 里 `bg` 是 `ansi_default`、`Screen.styles.background`
保住 `ansi=-1`、`$td-bg` 变量别被解析成色值，以及**Textual 默认挂的
...
（前景色的 `default` 照旧换，那是 Textual 控件的常规行为）。
...
| 更多透明版 | 现在只有 `A-T` / `P3-T` 两套（用户点名的两个）。它是一条通用规则（`_transparent(base)`），要加别的套只是再调一次 —— 但那意味着更多"深色主题配亮底板"的组合，不如按需加 |
...
| 选中项 = `▌` + 反白 | `.option.selected { background: $td-accent; color: $td-bg }` + `width: 1fr` | 用反白而不是"强调色当底 + 手调正文色"：反白在**每一套主题**下都自带对比，不用对 12 套逐一校对（透明版的 `$td-bg` 是 `ansi_default`，反白那一格的底色于是也交给终端 —— 这是这套做法白捡的好处）。`▌` 是给单色终端的形状信号；`width: 1fr` 是让那块底**铺满整行**的关键（不铺就只有文字那么长） |
...
| 命令 | 数据从哪来 | 界面形态 |
|---|---|---|
| `/theme` | `theme.py` 的 12 套（`ORDER` / `THEMES`） | **不带参数弹选择面板**：`↑↓` 选、`Enter` 换、`Esc` 取消；带参数照旧走 `theme.resolve` |
...
**`/theme` 原本在 18 的草稿里被排除**（理由是"12 套各有中文名、换错了一眼看得出来"）。
后来一起做了，因为那句话只否掉了一半：配色换错确实看得见，但**清单上那些名字本来也只是
为了照着打一遍** —— 12 套里的序号最容易记错（`/theme 6` 是 `A`，不是 `P6`；裁掉四套
色卡之后 `/theme 7` 甚至变成了 `B`）。而"看得见"这条差别没消失，它落在 18.2 那张表的
**关不关面板**那一格里。
...
| `/model` | 停在**当前的下一个**（打开它的人几乎总是想换一个，按 `Esc` 才是"留在原地"） | **不关**：等 runtime 那条 notice |
...
- **工具描述与工具结果**（模型看的）；
- **12 套配色的中文名**：它们是长在主题上的数据，英文名并存（`name_en` / `source_en`），
