...
### 3.4 `ui` —— 只给界面的东西，四种 `kind`

它和 `t:"event"` 的分工是**一件事**：**`ui` 不进审计**。四种 `kind` 都服从这一条。
...
会话规模 / **当前模型与它的窗口**）。它存在的原因很具体：**这些状态住在子进程里**
（`session.metadata` 和 `PermissionPolicy`），而你是另一个进程 —— 没有这条消息，
"agent 现在在做哪几件事""我现在放行了什么""它在用哪个模型"就只剩"另开一个终端跑
`--skills` / `--audit`"这一条出口。

**什么时候发**：开场一条（紧跟 `session_load`，**这一条带 `skill_catalog`**），
以及每一条 `tool_result` 之后（`todo_write` / `load_skill` 改的正是那两块），
回合收尾时再补一条。3.9 那条 `set_model` 处理完也补一条。**别按工具名去挑** ——
那会让协议层认识具体工具。
...
**`kind:"status"` 和 `kind:"tools"` 是"回答"**：它们分别回应 3.10 那两条入站消息，
**只在被问的时候才发**（前者要读一遍审计日志、后者要遍历工具注册表并问一次策略）。
和 `state` 那个"每轮都补一份"的常驻快照不是一类东西，所以没有合成一种 `kind`。
...
**它不该混进对话流**（`status` / `tools` 除外）：任务列表每更新一次就往会话里插一段，
会把"你问的 + 它答的"冲稀。它属于常驻面板（左栏那种）。而 `status` / `tools` 是
...
当前模型）**不必再抄一遍进对话流** —— 那只会让开局看起来像一屏日志。
...
**两条都是只读的，而且都不打断当前这一轮。** `/status` 最有用的时刻恰恰是"它跑着，
...
而第二份事实漂掉的症状永远是"看起来正常，其实不一样"。第 4、6 条是同一个理由的另一个
方向 —— 那些目录布局是 store / sink 的实现细节，不是协议的一部分。
...
那两条和其余所有出站消息的分界线。**`status` / `tools` 也走 `on_message`**：它们
